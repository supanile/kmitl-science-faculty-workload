...
const statusConfig: Record<
  "pending" | "approved" | "rejected" | "returned" | "draft",
  StatusConfig
> = {
  draft: {
    label: "",
    ...
    buttonBgColor:
      "bg-blue-100/70 hover:bg-blue-100 active:bg-blue-200 dark:bg-blue-500/10 dark:hover:bg-blue-500/15 dark:active:bg-blue-500/25",
  ...
  pending: {
    ...
    buttonBgColor:
      "bg-orange-100/70 hover:bg-orange-100 active:bg-orange-200 dark:bg-orange-500/10 dark:hover:bg-orange-500/15 dark:active:bg-orange-500/25",
  ...
  approved: {
    ...
    buttonBgColor:
      "bg-green-100/70 hover:bg-green-100 active:bg-green-200 dark:bg-green-500/10 dark:hover:bg-green-500/15 dark:active:bg-green-500/25",
  ...
  rejected: {
    ...
    buttonBgColor:
      "bg-red-100/70 hover:bg-red-100 active:bg-red-200 dark:bg-red-500/10 dark:hover:bg-red-500/15 dark:active:bg-red-500/25",
  ...
  returned: {
    ...
    buttonBgColor:
      "bg-purple-100/70 hover:bg-purple-100 active:bg-purple-200 dark:bg-purple-500/10 dark:hover:bg-purple-500/15 dark:active:bg-purple-500/25",
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2 mb-2 sm:mb-3">
        <div className="flex-1">
          <p
            className={`text-sm sm:text-base font-extrabold line-clamp-1 ${statusInfo.codeTxtColor}`}
          >
            {courseCode}
          </p>
        </div>
        {/* Edit hint icon — show on hover only if editable */}
        {isEditable && (
          <Pencil
            className={`h-3.5 w-3.5 shrink-0 mt-0.5 opacity-0 group-hover:opacity-60
        transition-opacity duration-150 ${statusInfo.codeTxtColor}`}
          />
        )}
      </div>
      ...
      {status !== "draft" && (
        ...
        >
          {/* Status Badge */}
          <div
            className={`flex items-center justify-start gap-2 py-2 px-3 rounded-full border ${statusInfo.cardBorderColor} ${statusInfo.bgColor}`}
          >
            <div className={statusInfo.textColor}>{statusInfo.icon}</div>
            <span
              className={`text-xs sm:text-sm font-semibold ${statusInfo.textColor}`}
            >
              {statusInfo.label}
            </span>
          </div>
